...
async function generateReferenceCode(supabase: ReturnType<typeof createServiceClient>): Promise<string | null> {
  for (let i = 0; i < 10; i++) {
...
export async function POST(req: NextRequest) {
  ...
  const { name, phone, items, totalPrice, memo, ipaymuEnabled = true } = body

  if (!name || !phone || totalPrice === undefined ||
  ...
  await notifyCustomerOrderReceived({ phone, name, referenceCode, totalPrice })

  // Call VPS proxy to create iPaymu QRIS payment
  const proxyUrl = process.env.IPAYMU_PROXY_URL?.trim()
  const proxySecret = process.env.PROXY_SECRET?.trim()
  const baseUrl = (process.env.NEXT_PUBLIC_BASE_URL ?? 'https://open-po.guudo.id').trim()

  let qrisUrl: string | null = null

  let proxyDebug: unknown = null
  ...
  if (ipaymuEnabled && proxyUrl && proxySecret) {
    try {
      ...
      console.log('[iPaymu proxy response]', JSON.stringify(ipaymuData))
      proxyDebug = ipaymuData
      if (ipaymuData?.Data) {
        const trxId = ipaymuData.Data.TransactionId ? String(ipaymuData.Data.TransactionId) : null
        qrisUrl = ipaymuData.Data.QrString ?? null
        if (trxId) {
          await supabase.from('orders').update({ ipaymu_trx_id: trxId }).eq('id', order.id)
        }
      }
    } catch (err) {
      console.error('[iPaymu proxy error]', err)
      proxyDebug = { error: String(err) }
    }
  } else {
    console.warn('[iPaymu] IPAYMU_PROXY_URL or PROXY_SECRET not set')
    proxyDebug = { error: 'env vars missing' }
  }

  return NextResponse.json({ success: true, referenceCode, qrString: qrisUrl, proxyDebug }, { status: 201 })
}
